...
/**
 * GPU-driven renderer used during direct face editing.
 *
 * The committed mesh is uploaded once to persistent VBO/IBO buffers. During a
 * drag gesture, only radial and axial scale uniforms change. No vertex arrays,
 * triangle groups or selection buffers are rebuilt until a definitive BRep is
 * committed by FreeCAD/OpenCASCADE.
 */
class GpuFaceDrivenCadRenderer : GLSurfaceView.Renderer {
    ...
    @Volatile private var pendingPreviewLength = Float.NaN
    @Volatile private var pendingPreviewDiameter = Float.NaN
    @Volatile private var previewDirty = false
    @Volatile private var clearPreviewPending = false

    @Volatile var selectedFace: EditableCadFace = EditableCadFace.NONE
        private set

    private data class CpuChunk(
    ...
    private var meshChunks: List<GpuChunk> = emptyList()
    private var topChunks: List<GpuChunk> = emptyList()
    private var sideChunks: List<GpuChunk> = emptyList()

    private var meshProgram = 0
    private var flatProgram = 0
    private var width = 1
    private var height = 1
    private var radialScale = 1f
    private var axialScale = 1f
    ...
    private val meshVertexShader = """
        ...
        void main() {
            vec3 p = aPosition;
            p.xy = uRadialCenter + (p.xy - uRadialCenter) * uRadialScale;
            p.z = uAnchorZ + (p.z - uAnchorZ) * uAxialScale;
    ...
    fun setLivePreview(targetLengthMm: Float, targetDiameterMm: Float) {
        if (!targetLengthMm.isFinite() || !targetDiameterMm.isFinite()) return
        pendingPreviewLength = targetLengthMm
        pendingPreviewDiameter = targetDiameterMm
        previewDirty = true
    }

    fun clearLivePreview() {
        clearPreviewPending = true
    }
    ...
    fun pickFace(screenX: Float, screenY: Float): EditableCadFace {
        ...
        if (a > 1e-8f && discriminant >= 0f) {
            ...
            for (t in candidates) {
                ...
                val z = near[2] + rayZ * t
                if (z in bounds.minZ..bounds.maxZ) {
    ...
    fun hitManipulator(screenX: Float, screenY: Float, radiusPx: Float): Boolean {
        ...
        return distanceToSegment(screenX, screenY, a[0], a[1], b[0], b[1]) <= radiusPx
    }
    ...
    override fun onDrawFrame(gl: GL10?) {
        pendingMesh?.let { mesh ->
            deleteChunks(meshChunks)
            deleteChunks(topChunks)
            deleteChunks(sideChunks)
            activeMesh = mesh
            meshChunks = uploadChunks(buildCpuChunks(mesh) { true })
            topChunks = uploadChunks(buildCpuChunks(mesh) { triangleIsTop(mesh, it) })
            sideChunks = uploadChunks(buildCpuChunks(mesh) { triangleIsSide(mesh, it) })
            radialScale = 1f
            axialScale = 1f
            rebuildGuides(mesh)
            rebuildArrow(mesh, selectedFace)
            pendingMesh = null
            clearPreviewPending = false
            previewDirty = false
        }

        if (clearPreviewPending) {
            radialScale = 1f
            axialScale = 1f
            activeMesh?.let { rebuildArrow(it, selectedFace) }
            clearPreviewPending = false
            previewDirty = false
        } else if (previewDirty) {
            activeMesh?.let { mesh ->
                val baseLength = (mesh.maxZ - mesh.minZ).coerceAtLeast(MIN_SPAN)
                val baseDiameter = max(mesh.maxX - mesh.minX, mesh.maxY - mesh.minY).coerceAtLeast(MIN_SPAN)
                axialScale = pendingPreviewLength.coerceAtLeast(MIN_SPAN) / baseLength
                radialScale = pendingPreviewDiameter.coerceAtLeast(MIN_SPAN) / baseDiameter
                rebuildArrow(mesh, selectedFace)
            }
            previewDirty = false
        }
    ...
    private fun drawSelection(chunks: List<GpuChunk>) {
    ...
    private fun drawChunks(chunks: List<GpuChunk>, r: Float, g: Float, b: Float, a: Float) {
        val mesh = activeMesh ?: return
        ...
        val centerLocation = GLES20.glGetUniformLocation(meshProgram, "uRadialCenter")
        val anchorLocation = GLES20.glGetUniformLocation(meshProgram, "uAnchorZ")
        val radialLocation = GLES20.glGetUniformLocation(meshProgram, "uRadialScale")
        val axialLocation = GLES20.glGetUniformLocation(meshProgram, "uAxialScale")
        ...
        GLES20.glUniform2f(centerLocation, (mesh.minX + mesh.maxX) * 0.5f, (mesh.minY + mesh.maxY) * 0.5f)
        GLES20.glUniform1f(anchorLocation, mesh.minZ)
        GLES20.glUniform1f(radialLocation, radialScale)
        GLES20.glUniform1f(axialLocation, axialScale)
    ...
    private fun drawFlatSlice(buffer: FloatBuffer, first: Int, count: Int, mode: Int, r: Float, g: Float, b: Float, a: Float, lineWidth: Float) {
    ...
    private fun rebuildArrow(mesh: NativeSceneMesh?, face: EditableCadFace) {
        ...
        val headLength = effectiveBounds(actual).maxDimension * 0.16f
        ...
        val ring = arrayOf(offset(neck, u, radius), offset(neck, v, radius), offset(neck, u, -radius), offset(neck, v, -radius))
    ...
    private fun manipulatorEndpoints(face: EditableCadFace): Pair<FloatArray, FloatArray>? {
        ...
        val length = bounds.maxDimension * 0.72f
        return when (face) {
            EditableCadFace.TOP -> floatArrayOf(cx, cy, bounds.maxZ) to floatArrayOf(cx, cy, bounds.maxZ + length)
            EditableCadFace.SIDE -> floatArrayOf(bounds.maxX, cy, cz) to floatArrayOf(bounds.maxX + length, cy, cz)
            EditableCadFace.NONE -> null
        }
    }

    private data class Bounds(val minX: Float, val minY: Float, val minZ: Float, val maxX: Float, val maxY: Float, val maxZ: Float) {
        val maxDimension: Float
            get() = maxOf(maxX - minX, maxY - minY, maxZ - minZ).coerceAtLeast(MIN_SPAN)
    }

    private fun effectiveBounds(mesh: NativeSceneMesh): Bounds {
        val cx = (mesh.minX + mesh.maxX) * 0.5f
        val cy = (mesh.minY + mesh.maxY) * 0.5f
        return Bounds(
            cx + (mesh.minX - cx) * radialScale,
            cy + (mesh.minY - cy) * radialScale,
            mesh.minZ,
            cx + (mesh.maxX - cx) * radialScale,
            cy + (mesh.maxY - cy) * radialScale,
            mesh.minZ + (mesh.maxZ - mesh.minZ) * axialScale
        )
    }
    ...
    private fun buildCpuChunks(mesh: NativeSceneMesh, include: (IntArray) -> Boolean): List<CpuChunk> {
        val result = ArrayList<CpuChunk>()
        ...
        fun flush() {
            ...
            val vertices = localVertices.copyOf(vertexCount * 6)
            val indices = localIndices.copyOf(indexCount)
            result += CpuChunk(
    ...
    private fun uploadChunks(cpuChunks: List<CpuChunk>): List<GpuChunk> = cpuChunks.map { chunk ->
        val ids = IntArray(2)
        GLES20.glGenBuffers(2, ids, 0)
        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, ids[0])
        chunk.vertices.position(0)
        GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, chunk.vertexBytes, chunk.vertices, GLES20.GL_STATIC_DRAW)
        GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, ids[1])
        chunk.indices.position(0)
        GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER, chunk.indexBytes, chunk.indices, GLES20.GL_STATIC_DRAW)
        GpuChunk(ids[0], ids[1], chunk.indexCount)
    }.also {
        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, 0)
        GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, 0)
    }

    private fun deleteChunks(chunks: List<GpuChunk>) {
        if (chunks.isEmpty()) return
        val ids = IntArray(chunks.size * 2)
        chunks.forEachIndexed { index, chunk ->
            ids[index * 2] = chunk.vertexBuffer
            ids[index * 2 + 1] = chunk.indexBuffer
        }
