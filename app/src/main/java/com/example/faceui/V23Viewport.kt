...
@Composable
internal fun RowScope.V23Viewport(
    ...
    Box(Modifier.weight(1f).fillMaxHeight()) {
        AndroidView(
            factory = { context -> FaceDrivenCadSurfaceView(context).also { view -> controller.surface = view; onSurfaceReady(view) } },
            update = { view ->
                view.onFaceSelected = { face ->
                    if (!controller.loading && directCylinder) {
                        controller.selectedFace = face
                        view.setSelectedFace(face)
                        controller.setStatus(when (face) {
                ...
                view.onParameterPreview = { face, value ->
                    if (!controller.loading) controller.setStatus(when (face) {
                ...
                view.onParameterCommit = { face, value ->
                    if (!controller.loading && directCylinder && program != null && firstFeature != null) {
                        val diameter = if (face == EditableCadFace.SIDE) value.toDouble() else firstSketch?.parameters?.get("diameter") ?: firstFeature.parameters["diameter"] ?: 34.93
                        val depth = if (face == EditableCadFace.TOP) value.toDouble() else firstFeature.parameters["depth"] ?: 40.0
                        runCatching { program.updateBaseCylinder(diameter, depth) }
                            .onSuccess { controller.commit(it, false, "${firstFeature.label} actualizado desde la cara", V23SelectionType.FEATURE, firstFeature.id) }
                            .onFailure { controller.reportError(v23Failure(it)) }
                    }
                }
                view.onPlanarFacePicked = onPlanarFacePicked
                controller.document?.let { document -> if (view.tag != document.token) { view.tag = document.token; view.setMesh(document.mesh, document.fit) } }
        ...
        AnimatedVisibility(visible = !treeVisible, modifier = Modifier.align(Alignment.CenterStart), enter = fadeIn() + slideInHorizontally(initialOffsetX = { -it }), exit = fadeOut() + slideOutHorizontally(targetOffsetX = { -it })) { V23ViewportHandle("Árbol", "▶", onShowTree, false) }
        AnimatedVisibility(visible = !rightVisible, modifier = Modifier.align(Alignment.CenterEnd), enter = fadeIn() + slideInHorizontally(initialOffsetX = { it }), exit = fadeOut() + slideOutHorizontally(targetOffsetX = { it })) { V23ViewportHandle("Prop.", "◀", onShowRight, true) }
        if (!dockVisible) {
        ...
        Surface(modifier = Modifier.align(Alignment.BottomCenter).padding(bottom = if (dockVisible) 206.dp else 48.dp), color = Color.White.copy(alpha = 0.93f), contentColor = V23Secondary, shape = RoundedCornerShape(7.dp), shadowElevation = 2.dp) {
            Text(when { planarPickMode -> "Toque una cara plana para crear un plano paralelo"; directCylinder -> "Toque una cara y arrastre la flecha amarilla"; else -> "Órbita: 1 dedo · paneo: 2 dedos · zoom: pellizco" }, Modifier.padding(horizontal = 10.dp, vertical = 6.dp), color = if (planarPickMode) Color(0xFF8A4B08) else V23Secondary, fontSize = 9.sp, fontWeight = FontWeight.Medium)
        }
        ...
        controller.error?.let { V23ErrorCard(it, controller::clearError, Modifier.align(Alignment.BottomStart).padding(12.dp)) }
    }
}
...
private fun V23ViewportHandle(label: String, arrow: String, onClick: () -> Unit, right: Boolean) {
    Surface(modifier = Modifier.clickable(onClick = onClick), color = V23Accent, contentColor = Color.White, shape = if (right) RoundedCornerShape(topStart = 8.dp, bottomStart = 8.dp) else RoundedCornerShape(topEnd = 8.dp, bottomEnd = 8.dp), shadowElevation = 4.dp) {
        Column(Modifier.padding(horizontal = 7.dp, vertical = 15.dp), horizontalAlignment = Alignment.CenterHorizontally) { Text(arrow, color = Color.White, fontWeight = FontWeight.Bold); Text(label, color = Color.White, fontSize = 7.sp, fontWeight = FontWeight.Bold) }
    }
}
